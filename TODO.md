...
| STK-0010 | Split Cloudflare manifests into public Static Assets at `/public/_manifest` and private Worker-managed data at `/_manifest`                                                                                                                                    | 🟨 In Progress  | Public manifest should be a static asset that exposes only public routes; private manifest should move off `/manifest` and remain Worker-managed under `/_manifest`.                                                                                                                               |
| STK-0011 | Make Cloudflare preview use correct fetch URLs and JSON paths, including `STATIK_USE_INDEX_JSON` handling for public assets                                                                                                                                    | 🟨 In Progress  | The preview UI must stop guessing `index.json` paths for Cloudflare routes and instead show/fetch the actual asset URL for public routes while keeping private Worker routes correct.                                                                                                             |
