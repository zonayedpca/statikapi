# Cloudflare Pages, Workers, and R2
...
## For Larger Projects — Use R2 or the Cloudflare adapter

If your StatikAPI project produces many JSON files or large objects, consider either:

- hosting a local `api-out/` build in **Cloudflare R2**, or
- using the dedicated `cloudflare-adapter` template for a Worker + Static Assets + private R2 deployment

R2 is Cloudflare’s object storage service, similar to AWS S3, designed for large data sets and high availability.

### Upload a local build to R2
...
- Cloudflare-specific runtime limits

use the dedicated [Cloudflare Worker + R2 adapter](../serverless/cloudflare-adapter) instead of a plain `api-out/` upload flow.
...
- You can also host your APIs directly with **StatikAPI Cloud** _(coming soon)_ — a managed hosting solution built for static JSON APIs, automatic builds, and versioned deployments.  
