...
- **Serverless mode** using the **Cloudflare Worker adapter** (Worker + Static Assets for public routes, private R2 + KV, with a build webhook)
...
- **Template**: `basic`, `dynamic`, `remote-data`, or  
  `cloudflare-adapter` (Cloudflare Worker + Static Assets + private R2 + KV + build webhook)
