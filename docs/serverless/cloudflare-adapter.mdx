...
In the scaffolded project, `pnpm deploy` is the safer default because it runs the StatikAPI build first and can optionally seed private outputs after deploy if `STATIK_DEPLOY_ORIGIN` is configured.
...
- `STATIK_DEPLOY_ORIGIN` points at the final deployed Worker or custom domain if you want automatic post-deploy private-output seeding
...
3. Set the deployment command to `pnpm deploy` if you want this wrapper's build-first and best-effort seed behavior.
...
- if `STATIK_DEPLOY_ORIGIN` is set, tries a post-deploy `POST /` so private outputs are seeded immediately
- if that seed step cannot run or the deployed Worker is not ready yet, `pnpm deploy` still succeeds and prints a warning with the manual fallback command
...
7. Optionally set `STATIK_DEPLOY_ORIGIN=https://your-app.example.com` in `.dev.vars` if you want deploy to seed private outputs automatically.
8. Run:
...
If you do not set `STATIK_DEPLOY_ORIGIN`, seed private outputs after deploy with:
